...
// TODO set an exception if we define a label from a reserved name
...
namespace dcpu11
{
    ...
    */
    ...
    Assembler::Assembler()
    {
        ...
        // Other variables
        addVariable("PUSH", Variable(AD_PUSH));
        addVariable("PEEK", Variable(AD_PEEK));
        addVariable("POP", Variable(AD_POP));
        addVariable("SP", Variable(AD_SP));
        addVariable("PC", Variable(AD_PC));
        addVariable("OV", Variable(AD_OV));
    }
...
//        #endif
    ...
    bool Assembler::assembleStream(std::istream & is)
    {
        while(!is.eof())
        {
            ...
            std::string line;
            char c = 0;
            // TODO make the newline char work on other OS than Linux
            while(c != '\n')
            {
                if(is.eof())
                    break;
    ...
    bool Assembler::getOperandCode(const Operand & op, u16 & code)
    {
        if(op.lookup) // [...]
        {
    ...
    bool Assembler::assembleBasicOp(u16 opcode, Operand a, Operand b)
    {
        if(!getOperandCode(a, a.code)) return false;
        if(!getOperandCode(b, b.code)) return false;
        ...
        if(isAdvancePC(a.code) && !assembleFullWordOperand(a))
            return false;
        if(isAdvancePC(b.code) && !assembleFullWordOperand(b))
            return false;
        ...
        for(usesIt = m_labelUses.begin(); usesIt != m_labelUses.end(); usesIt++)
        {
            ...
            for(useListIt = useList.begin(); useListIt != useList.end(); useListIt++)
            {
                ...
                else
                {
                    m_pos = use.col;
                    m_row = use.row;
                    setException("Undefined label '" + use.name + "'");
                    return false;
                }
            }
        }
